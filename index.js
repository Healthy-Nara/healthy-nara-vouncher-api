...
app.patch('/api/bookings/:id/assign', authMiddleware, roleMiddleware(['admin', 'staff']), async (req, res) => {
  try {
    ...
    const children = parent?.children?.length > 0 ? parent.children : [{ childName: booking.customerName }];
      ...
      for (const child of children) {
        try {
          ...
          if (!existingReport) {
            ...
            console.log('[Assign] Created report for', child.childName, 'on', date);
          }
        } catch (createErr) {
          console.error('[Assign] DailyReport create error:', createErr.message);
        }
      }
    }
