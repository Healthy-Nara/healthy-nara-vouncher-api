...
app.get(
  ...
  async (req, res) => {
    try {
      ...
      const fInvoices = invoices.filter(inRange);
      const fLeads = leads.filter(inRange);
      const fBookings = bookings.filter(inRange);
      const fExpenses = expenses.filter(inRange);
      ...
      const stats = {
        ...
        totalRevenue: fInvoices.reduce(
          (sum, inv) => sum + inv.amount + (inv.platformFee || 0),
          0,
        ),
        totalPayouts: fInvoices.reduce((sum, inv) => sum + inv.amount, 0),
        totalExpenses,
        totalProfit:
          fInvoices.reduce((sum, inv) => sum + (inv.platformFee || 0), 0) -
          totalExpenses,
        pendingPayments: fInvoices
          .filter((i) => i.customerPaymentStatus === "Pending")
          .reduce((sum, inv) => sum + inv.amount + (inv.platformFee || 0), 0),
        accountsReceivable: fInvoices
          .filter(
            (i) =>
          ...
          .reduce((sum, inv) => sum + inv.amount + (inv.platformFee || 0), 0),
        pendingPayouts: fInvoices
          .filter((i) => i.caregiverPayoutStatus === "Pending")
          .reduce((sum, inv) => sum + inv.amount, 0),
        completedInvoices: fInvoices.filter((i) => i.status === "Completed")
          .length,
        // Lead stats (counts filtered by createdAt)
        totalLeads: fLeads.length,
        newLeads: fLeads.filter((l) => l.stage === "New").length,
        contactedLeads: fLeads.filter((l) => l.stage === "Contacted").length,
        saleClosedLeads: fLeads.filter((l) => l.stage === "Sale Closed").length,
        activeCustomers: leads.filter((l) => l.stage === "Active Customer")
          .length,
        lostLeads: fLeads.filter((l) => l.stage === "Lost").length,
        // Booking stats (counts filtered, snapshots all-time)
        totalBookings: fBookings.length,
        pendingBookings: fBookings.filter(
          (b) => b.status === "Pending NA Selection",
        ).length,
        assignedBookings: bookings.filter((b) => b.status === "Assigned")
          .length,
        completedBookings: fBookings.filter((b) => b.status === "Completed")
          .length,
        activeNAs: new Set(
          bookings
        ...
        // Invoice status stats (filtered by createdAt)
        draftInvoices: fInvoices.filter((i) => i.invoiceStatus === "Draft")
          .length,
        createdInvoices: fInvoices.filter((i) => i.invoiceStatus === "Created")
          .length,
        sentInvoices: fInvoices.filter((i) => i.invoiceStatus === "Sent").length,
        confirmedInvoices: fInvoices.filter(
...
app.get(
  ...
  async (req, res) => {
    try {
      ...
      const expenses = await Expense.find(dateQuery).sort({ createdAt: -1 });
  ...
  async (req, res) => {
    try {
      ...
      await createLog(
        ...
        `Expense ${category} ${amount} created`,
      );
  ...
  async (req, res) => {
    try {
      ...
      await createLog(
        ...
        "Expense",
        expense._id.toString(),
        `Expense ${expense.category} ${expense.amount} updated`,
      );
  ...
  async (req, res) => {
    try {
      ...
      await createLog(
        ...
        "Expense",
        req.params.id,
        `Expense ${expense.category} ${expense.amount} deleted`,
      );
